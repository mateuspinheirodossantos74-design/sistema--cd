...
def limpar_tabela(nome_tabela):
    try:
        ...
        st.error(f"Erro ao limpar tabela {nome_tabela}: {e}")
        return False
...
def render():
    ...
    with aba1:
        ...
        # 🔥 LIMPAR TABELA DEMANDA
        st.markdown("### Atenção")
        confirmar = st.checkbox("Confirmar limpeza da tabela DEMANDA")

        if st.button(" Limpar tabela de demanda"):
            if confirmar:
                if limpar_tabela("demanda"):
                    st.success("Tabela de demanda limpa!")
                    st.cache_data.clear()
                    st.rerun()
            else:
                st.warning("Confirme antes de apagar!")
    ...
    with aba2:
        ...
        # 🔥 LIMPAR TABELA CONFERENTES
        st.markdown("### Atenção")
        confirmar2 = st.checkbox("Confirmar limpeza da tabela CONFERENTES")

        if st.button("Limpar tabela de conferentes"):
            if confirmar2:
                if limpar_tabela("conferentes"):
                    st.success("Tabela de conferentes limpa!")
                    st.cache_data.clear()
                    st.rerun()
            else:
                st.warning("Confirme antes de apagar!")
